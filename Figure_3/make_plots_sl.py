...
data_ET = numpy.loadtxt('ET_output.txt', delimiter=',')
data_ET = data_ET[data_ET[:,2] < 0]
data_ET = numpy.array(sorted(data_ET, key = lambda x: x[1]))
data_ET = numpy.array(sorted(data_ET, key = lambda x: x[0]))
...
ax4 = axes[2]
exclusions1 = data_ET[:,3]
exclusions2 = data_LISA[:,3]
exclusions_comb = numpy.amin(numpy.array([exclusions1, exclusions2]), axis=0)
print(exclusions_comb)
sc_plot = ax4.scatter(f_stars, alphas, s=15, marker='o', c=exclusions_comb, norm=matplotlib.colors.LogNorm(vmax=1, vmin=1E-20))

log_grid_excs = numpy.log10(numpy.resize(exclusions_comb,[25,50]).T)
result = ndimage.zoom(log_grid_excs, zoom_level, order=3, prefilter=False)
result = 10**result
ctours = ax4.contour(fstar_valuesZ, alpha_valuesZ, result, levels=[1E-15, 1E-12, 1E-9, 1E-6, 1E-3], norm=matplotlib.colors.LogNorm(vmax=1, vmin=1E-20))
...
f_stars = data_ET[:,0]
alphas = data_ET[:,1]
exclusions = data_ET[:,3]
sc_plot = ax3.scatter(f_stars, alphas, s=15, marker='o', c=exclusions, norm=matplotlib.colors.LogNorm(vmax=1, vmin=1E-20))
log_grid_excs = numpy.log10(numpy.resize(exclusions,[25,50]).T)
result = ndimage.zoom(log_grid_excs, zoom_level, order=3, prefilter=False)
result = 10**result
ctours = ax3.contour(fstar_valuesZ, alpha_valuesZ, result, levels=[1E-15, 1E-12, 1E-9, 1E-6, 1E-3], norm=matplotlib.colors.LogNorm(vmax=1, vmin=1E-20))
...
fig.savefig('figureA2_sl.pdf', bbox_inches='tight')
fig.savefig('figureA2_sl.png', bbox_inches='tight')
